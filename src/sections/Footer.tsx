...
export default function Footer() {
  return (
    <footer
      className="border-t border-white/10 px-6 pb-8 pt-16 text-[12px] text-foreground/60 md:px-10"
      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid grid-cols-2 gap-10 pb-12 sm:grid-cols-3 lg:grid-cols-4">
          <div>
            ...
            <p className="mt-2 max-w-[240px] text-[12px] leading-[1.5] text-foreground/60">
          ...
          {COLS.map((c) => (
            <div key={c.heading}>
              <h3 className="mb-3 text-[12px] font-semibold text-foreground">
                {c.heading}
              </h3>
              {c.links.map((l) =>
                l.static ? (
                  <span
                    key={l.label}
                    className="block py-1 leading-[2] text-foreground/60"
                  >
                    {l.label}
                  </span>
                ) : (
                  <a
                    key={l.label}
                    href={l.href}
                    className="block py-1 leading-[2] text-foreground/60 transition-colors hover:text-foreground"
                  >
                    {l.label}
                  </a>
                )
        ...
        <div className="flex flex-col items-start justify-between gap-4 py-6 text-[12px] text-foreground/60 sm:flex-row sm:items-center">
